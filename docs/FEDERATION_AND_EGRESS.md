...
- **Inside a company** the hub is trusted: five permission layers, duplex
  bus, ledger.
- **Across the grid** nobody trusts anybody's hub: sharing contracts,
  notarization, and benchmark gates.

## External egress — every spoke's capability

Any spoke may expose **select data to external hubs and spokes** — beyond
hub & peer grants. Egress is not a data dump; it is contract-shaped:
...
- **Full node** — your company: this hub + your spokes.
...
`hub.notary` anchors salted Merkle roots to external consensus
