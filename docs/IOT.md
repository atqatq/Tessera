...
IoT is not an integration; it is a declared property of every spoke.

## Per-spoke telemetry

Every spoke's manifest declares the telemetry that is *meaningful* to its
domain (a full table in docs/SPOKES.md):
...
2. **`hub.ingest`** accepts MQTT/edge streams alongside CSV / XLSX / API;
...
4. **Spoke craft consumes it**: OEE counters feed TRANSFORM's SPC;
